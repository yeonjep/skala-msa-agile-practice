  ...
  <header class="app-header">
    <div class="header-inner">
      ...
      <router-link to="/" class="logo">
        <img src="@/assets/images/logo/main_logo.png" alt="한끼픽" class="logo-img" />
        <span class="logo-text">한끼픽</span>
      </router-link>

      <!-- 네비게이션 -->
      <nav class="nav-links" v-if="auth.isAuthenticated">
        <router-link to="/courses" class="nav-link" :class="{ active: $route.path.startsWith('/courses') }">상품</router-link>
        <router-link to="/enrollments" class="nav-link" :class="{ active: $route.path === '/enrollments' }">내 구독</router-link>
      </nav>
